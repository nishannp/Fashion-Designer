...
/**
 * Creates a single "styleboard" page image from a collection of hairstyle images.
 * @param imageData A record mapping hairstyle strings to their image data URLs.
 * @returns A promise that resolves to a data URL of the generated styleboard page (JPEG format).
 */
export async function createStyleboard(imageData: Record<string, string>): Promise<string> {
    ...
    ctx.fillText('Generated with Style Sync AI', canvasWidth / 2, 150);
    ...
    const hairstyles = Object.keys(imageData);
    const loadedImages = await Promise.all(
        Object.values(imageData).map(url => loadImage(url))
    );

    const imagesWithHairstyles = hairstyles.map((hairstyle, index) => ({
        hairstyle,
    ...
    const reversedImages = [...imagesWithHairstyles].reverse();
    reversedImages.forEach(({ hairstyle, img }, reversedIndex) => {
        // Calculate the original index to determine grid position
        const index = imagesWithHairstyles.length - 1 - reversedIndex;
        ...
        ctx.fillText(hairstyle, 0, captionY);
